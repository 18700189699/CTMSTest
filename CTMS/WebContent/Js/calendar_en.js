...
function Iframe(){
    ...
    strIframe+="&nbsp;</td></tr>"+
    "    <tr>"+
    "    <td align='left'><input name=today type=button value='现在' onfocus='this.blur()' class='inputstyle' style='width: 30' title='当前日期'></td><td align='right'><input name='doOk' type=button value='确定' onfocus='this.blur()' class='inputstyle' style='width: 30'>&nbsp;<input type='button' name='closeCalendar' value='关闭' class='inputstyle' style='width: 30' title='取消'></td></tr></table>"+
    ...
    {	
		...
		with(document.all){
	        ...
	        if (WebCalendarTimeShow){
				...
				var now = new Date();
				popUp.document.all.hou.value = ((now.getHours()<10)?"0":"") + now.getHours();
				popUp.document.all.min.value = ((now.getMinutes()<10)?"0":"") + now.getMinutes();
				popUp.document.all.sec.value = ((now.getSeconds()<10)?"0":"") + now.getSeconds();
